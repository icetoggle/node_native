    ...
    namespace net
    {
        ...
        public:
            ...
            bool bind(const std::string& ip, int port) { 
				...
				return uv_tcp_bind(get<uv_tcp_t>(), (const struct sockaddr*)&addr, 0) == 0;
			}
            ...
            bool connect(const std::string& ip, int port, std::function<void(error)> callback)
            {
				...
				struct sockaddr_in addr;
				uv_ip4_addr(ip.c_str(), port, &addr);
                return uv_tcp_connect(new uv_connect_t, get<uv_tcp_t>(), (const struct sockaddr*)&addr, [](uv_connect_t* req, int status) {
